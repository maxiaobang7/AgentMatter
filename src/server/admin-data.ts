...
export async function getAdminDashboard() {
  await requireAdmin();
  const [counts, resources, operations, submissions] = await Promise.all([
    ...
    queryRows<(RowDataPacket & { id: number; github_url: string; category: string | null; component_path: string | null; display_name: string | null; hosts_json: string | string[] | null; note: string | null; status: "new" | "reviewing" | "accepted" | "rejected"; created_at: Date })[]>("SELECT id, github_url, category, component_path, display_name, hosts_json, note, status, created_at FROM submissions ORDER BY FIELD(status, 'new', 'reviewing', 'accepted', 'rejected'), created_at DESC LIMIT 100"),
  ]);
  return {
    counts,
